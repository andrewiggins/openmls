...
class Reader {
	...
	readTlsVecU32<T>(parseItem: Parser<T>): T[] {
	...
	readVec<T>(parseItem: Parser<T>): Vec<T> {
		const length = this.#readVariableLengthLength();
...
function parseLeafNodeIn(reader: Reader): LeafNodeIn {
	const encryptionKey: VLBytes = reader.readVLBytes();
	entities.push(Array.from(encryptionKey));
	const signatureKey: VLBytes = reader.readVLBytes();
	entities.push(Array.from(signatureKey));
	const credential: MlsCredential = {
		credential_type: reader.readUint16(),
		serialized_credential_content: reader.readVLBytes(),
	};
	entities.push({
		credential_type: credential.credential_type,
		serialized_credential_content: {
			vec: Array.from(credential.serialized_credential_content),
		},
	});
	const capabilities: Capabilities = {
		versions: reader.readVec(() => reader.readUint16()),
		ciphersuites: reader.readVec(() => reader.readUint16() as CipherSuite),
		extensions: reader.readVec(() => reader.readUint16()),
		proposals: reader.readVec(() => reader.readUint16()),
		credentials: reader.readVec(() => reader.readUint16()),
	};
	entities.push(capabilities);
	const leafNodeSource: LeafNodeSource = {};
	const extensions: Extensions = {};
	const signature = reader.readVLBytes();
	entities.push(Array.from(signature));
...
function parseKeyPackageIn(reader: Reader): KeyPackageIn {
	const protocol_version = reader.readUint16();
	entities.push(protocol_version);
	const ciphersuite = reader.readUint16() as CipherSuite;
	entities.push(ciphersuite);
	const initKey = reader.readVLBytes();
	entities.push(Array.from(initKey));
	const leafNode = parseLeafNodeIn(reader);
	const extensions = parseExtensions(reader);
	const signature = reader.readVLBytes();
	entities.push(Array.from(signature));
...
export function parseRegisterClientRequest(
	request: Uint8Array,
): RegisterClientRequest {
	const reader = new Reader(request);
	return {
		key_packages: reader.readObjectEntries(
			reader.readUint32(),
			(r) => {
				const key = r.readTlsByteVecU8();
				entities.push(Array.from(key));
				return key;
			},
			parseKeyPackageIn,
		),
